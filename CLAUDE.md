...
- **An NFC card can hand off to another module** (a Plex guid on a card plays through the Plex module). The NFC backend routes by URI scheme; the receiving module carries a `CardPlay.qml` that resolves and then `replaceWith`s its Player. A module reached this way must bypass its own auth/user gate — a card arrives already signed in as somebody, so it must never be diverted into a sign-in flow. It may still *offer* a switch once it is showing (Plex's `CardPlay.qml` does, so a Home can resume as the right profile without leaving the card), as long as that switch goes through the same PIN gate as any other. (Details: [ARCHITECTURE.md → Card Hand-off](ARCHITECTURE.md#card-hand-off-nfc--a-module).)
